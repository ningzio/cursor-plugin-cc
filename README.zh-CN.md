...
一个 [Claude Code](https://claude.ai/code) 插件——把任务派发给 [cursor-agent CLI](https://docs.cursor.com/cli)，在隔离的 git worktree 里跑。前台/后台都行，带完整的 job 控制（status / result / cancel）和跨 Claude 会话的安全边界。
...
当 Claude Code 在做主任务、你想同时让另一个 agent 干点活——做第二轮 review、跑长时间的 migration、独立写一遍实现——就可以把任务交给 cursor-agent。这个插件在 Claude Code 里加了 5 个 slash 命令，让这件事做起来安全：

- `/cursor:setup` — 检查 cursor-agent 是否安装并已登录
- `/cursor:dispatch <prompt>` — 派任务到 cursor-agent，在新建的 git worktree 里跑（前台或后台）
- `/cursor:status` — 列出当前 Claude 会话里的 job
...
4. **会话边界**。如果你同时开两个 Claude Code 窗口，`/cursor:status` 必须只显示当前会话的 job。SessionStart hook 把当前 Claude 会话 ID 注入环境变量，companion 据此过滤。

如果上面 4 个你都不需要，就别用这个插件——直接 `cursor-agent` 也挺好。

## 前置条件

- **Claude Code** — 从 <https://claude.ai/code> 安装
...
然后在 Claude Code 设置里（`~/.claude/settings.json` 或项目级 `.claude/settings.json`）加：
...
### 方式 B：通过 Claude Code 的插件管理器

如果未来上了 marketplace，可以：
...
安装完**重启 Claude Code 会话**，让 SessionStart hook 注册成功。
...
列出当前 Claude 会话的 job。`--all` 显示所有会话的 job。
...
| 变量 | 谁设的 | 用途 |
|---|---|---|
| `CURSOR_COMPANION_SESSION_ID` | SessionStart hook | 当前 Claude Code 会话 ID；用来过滤 job |
| `CLAUDE_PLUGIN_DATA` | Claude Code | 插件数据目录 |
| `CLAUDE_ENV_FILE` | Claude Code（SessionStart 时） | hook 把 export 语句写到这个文件，由 Claude Code source 进会话 |
| `CURSOR_COMPANION_AGENT_BINARY` | 你（测试） | 覆盖 cursor-agent 二进制路径 |
| `CURSOR_COMPANION_AGENT_BINARY_ARG0` | 你（测试） | 覆盖 argv[0] |

## 状态

**Phase 1** — 上面 5 个 slash 命令已稳定，有 109 个单元测试覆盖：原子状态写、dispatch reservation、PID 安全、会话过滤、dirty-worktree 拒 resume、stream-json 解析。

已知限制：

- 暂未发布到 marketplace——只能本地 clone 安装（方式 A）。
